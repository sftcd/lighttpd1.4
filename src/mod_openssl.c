...
#ifndef OPENSSL_NO_ECH
    /* keep these together for the moment, they may belong elsewhere, we'll see later */
    const buffer *ssl_echkeydir; /* cfg: an absolute directory name - see load_echkeys() for more */
    unsigned int ssl_echrefresh; /* cfg: default to 1800 second, otherwise a number of seconds, 0 == never */
    time_t  echkeyloadtime; /* stored time_t of last time key files loaded from dir */
    char ssl_echtrialdecrypt; /* cfg: "disable" to tun off, anything else or missing to turn on */
    char ssl_echonly; /* cfg: "enable" if you want this virtualhost to only be accessible if ECH worked */
#endif
...
typedef struct {
    ...
    short close_notify;
    unsigned short alpn;
    plugin_config conf; buffer *tmp_buf;
    log_error_st *errh;
} handler_ctx;
...
#ifndef OPENSSL_NO_ECH
/* 
 * load any ECH key files we find in the ssl_echkeydir directory 
 * files that match are called <foo>.pem 
 */
static int load_echkeys(server *srv, SSL_CTX *ssl_ctx, plugin_data *d)
{
    /*
     * Try load any good looking ECHConfig PEM files found in files in that directory
    ...
    plugin_config *s=&d->defaults;
    char *echdir=(char*)s->ssl_echkeydir->ptr;
    if (!echdir) {
        log_error(srv->errh, __FILE__, __LINE__, 
            "load_echkeys: bad input");
        return -1;
    ...
    if ((elen+7) >= PATH_MAX) {
        log_error(srv->errh, __FILE__, __LINE__, 
            "load_echkeys, error, name too long: %s", echdir);
        return -1;
    }
    DIR *dp;
    struct dirent *ep;
    ...
    if (dp==NULL) {
        log_error(srv->errh, __FILE__, __LINE__, 
            "load_echkeys, can't read directory: %s", echdir);
        return -1;
    }
    int somekeyworked=0;
    while ((ep=readdir(dp))!=NULL) {
        char pemfname[PATH_MAX];
        /*
         * If the file name matches *.pem, then try decode/enable that
         */
        ...
        if (nlen>4) {
            ...
            if ((elen+nlen+1+1)>=PATH_MAX) { /* +1 for '/' and of NULL terminator */
                closedir(dp);
                log_error(srv->errh, __FILE__, __LINE__, 
                    "load_echkeys, error, file name too long: %s", ep->d_name);
                return -1;
            }
            snprintf(pemfname,PATH_MAX,"%s/%s",echdir,ep->d_name);
            struct stat thestat;
            if (stat(pemfname,&thestat)==0) {
                if (SSL_CTX_ech_server_enable(ssl_ctx,pemfname)!=1) {
                    log_error(srv->errh, __FILE__, __LINE__, 
                        "load_echkeys failed for %s",pemfname);
                } else {
                    log_error(srv->errh, __FILE__, __LINE__, 
                        "load_echkeys worked for %s",pemfname);
                    somekeyworked=1;
                }
#if 0
                for (uint32_t i = 1; i < srv->config_context->used; ++i) {
                    plugin_ssl_ctx * const ctx = d->ssl_ctxs + i;
                    if (SSL_CTX_ech_server_enable(ctx->ssl_ctx,pemfname)!=1) {
                        log_error(srv->errh, __FILE__, __LINE__, 
                            "load_echkeys failed for %s context %d",pemfname,i);
                    } else {
                        log_error(srv->errh, __FILE__, __LINE__, 
                            "load_echkeys worked for %s context %d",pemfname,i);
                        somekeyworked=1;
                    }
                }
#endif
    ...
    if (somekeyworked==0) {
        log_error(srv->errh, __FILE__, __LINE__, 
            "load_echkeys failed for all keys but ECH configured - exiting");
        return -1;
    }
    closedir(dp);
    s->echkeyloadtime=time(0);
    return 0;
}
#endif
...
mod_openssl_merge_config_cpv (plugin_config * const pconf, const config_plugin_value_t * const cpv)
{
    switch (cpv->k_id) { /* index into static config_plugin_keys_t cpk[] */
      case 0: /* ssl.pemfile */
#ifndef OPENSSL_NO_ECH
          // I don't understand the assignment below pconf->pc is a struct
          // guess I need to read more of their config magic;-)
#endif
        if (cpv->vtype == T_CONFIG_LOCAL)
            pconf->pc = cpv->v.v;
        break;
...
#if defined(SSL_CLIENT_HELLO_SUCCESS) && defined(OPENSSL_NO_ECH)
static int
...
#ifndef OPENSSL_NO_ECH
        if (p && p->defaults.ssl_echkeydir!=NULL) {
            int rv=load_echkeys(srv,s->ssl_ctx,p);
            if (rv) {
                log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: load_echkeys failed returning %d", rv);
                return -1;
            }
            int numkeys=0;
            int ksrv=SSL_CTX_ech_server_key_status(s->ssl_ctx,&numkeys);
            if (ksrv!=1) {
                log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: SSL_ech_server_key_status failed, returning %d", ksrv);
            } else {
                log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: SSL_ech_server_key_status number of keys loaded %d", numkeys);
            }
        } else {
                log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: load_echkeys not called");
        }
        if (p->defaults.ssl_echtrialdecrypt==0) {
            log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: Leaving ECH trial decryption OFF");
        } else {
            log_error(srv->errh, __FILE__, __LINE__, 
                    "SSL: Turning ON ECH trial decryption");
...
#ifndef OPENSSL_NO_ECH
     ,{ CONST_STR_LEN("ssl.echkeydir"),
        T_CONFIG_STRING,
        T_CONFIG_SCOPE_SERVER }
     ,{ CONST_STR_LEN("ssl.echrefresh"),
        T_CONFIG_INT,
        T_CONFIG_SCOPE_SERVER }
     ,{ CONST_STR_LEN("ssl.echtrialdecrypt"),
        T_CONFIG_BOOL,
        T_CONFIG_SCOPE_SERVER }
     ,{ CONST_STR_LEN("ssl.echonly"),
        T_CONFIG_BOOL,
        T_CONFIG_SCOPE_CONNECTION }
#endif
...
#ifndef OPENSSL_NO_ECH
    defaults.ssl_echkeydir=NULL;
    defaults.ssl_echrefresh=1800; 
    defaults.echkeyloadtime=0; 
    defaults.ssl_echtrialdecrypt=0;
#endif
        ...
        for (; -1 != cpv->k_id; ++cpv) {
            ...
            switch (cpv->k_id) {
              ...
              case 10:/* ssl.stek-file */
                ...
                break;
#ifndef OPENSSL_NO_ECH
              case 11:/* ssl.echkeydir */
                if (!buffer_is_empty(cpv->v.b))
                    conf.ssl_echkeydir = cpv->v.b;
                break;
              case 12:/* ssl.echrefresh */
                conf.ssl_echrefresh=(cpv->v.u>0?cpv->v.u:0);
                break;
              case 13:/* ssl.echtrialdecrypt */
                conf.ssl_echtrialdecrypt=(cpv->v.u!=0?1:0);
                break;
              case 14:/* ssl.echonly */
                conf.ssl_echonly=(cpv->v.u!=0?1:0);
                break;
#endif
              default:/* should not happen */
                break;
        ...
        for (; -1 != cpv->k_id; ++cpv) {
            switch (cpv->k_id) {
              ...
              case 14:/* debug.log-ssl-noise */
                break;
#ifndef OPENSSL_NO_ECH
              case 15:/* ssl.echkeydir */
                if (!buffer_string_is_empty(cpv->v.b)) p->defaults.ssl_echkeydir = cpv->v.b;
                break;
#endif
              default:/* should not happen */
                break;
